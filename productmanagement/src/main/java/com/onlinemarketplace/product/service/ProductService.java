...
public class ProductService  {
	...
	/**
	 * Check cache product list if null then read from csv file.
	 * if unable to read data from csv file then go to data base to get 
	 * product list. 
	 * 
	...
	public Map<Long,Product> getProduct(){
		...
		if( products==null || products.isEmpty() || fromDB){
		...
		try{
			if(!products.isEmpty()){
				for (Product product : products) {
					if(product!=null){
						if(product.getCategory().getCategoryName().trim().equalsIgnoreCase(category.trim())){
							result.add(product);
						}				 
					}
				}
	...
	public List<Product> getBySubCategory(String category){
		...
		if(!products.isEmpty()){
			for (Product product : products) {
				if(product!=null){
					if(product.getSubCategory().getCategoryName().trim().equalsIgnoreCase(category.trim())){
						result.add(product);
					}				 
				}
			}
	 ...
	 * Get list of product from cashe list category value.
	 * 
	 * @param min : {@link Float}
	 * @param max : {@link Float}
	 *   
	 * @return List <{@link Product}> 
	 */
				...
				if(product!=null){
					...
					if(properties != null){
						for (ProductProperty productProperty : properties) {
							if(productProperty.getPropertyName().trim().toUpperCase().equals(propertyName.trim().toUpperCase()) && 
									productProperty.getPropertyValue().trim().toUpperCase().equals( propertyValue.trim().toUpperCase() ) ){
								result.add(product);
							}
						}
					}
	 ...
	 * 
	 */
	public List<Product> getByPropertyNameAndValue(List<Product> products , String propertyName , String propertyValue){
		List<Product> result = new ArrayList<Product>();
		if(!products.isEmpty()){
			for (Product product : products) {
				if(product!=null){
	...
	public List<Product> getByPropertyNameAndValue(Map<String,String> nameValuePair ) throws InvalidParameterException {
		List<Product> result = new ArrayList<Product>();
		...
		for (String key : nameValuePair.keySet()) {
			...
			if(value == null && value.isEmpty() ){
				logger.info("Value for the key "+key+" is not found in nameValuePair");
		    }
	...
	@SuppressWarnings("null")
	public List<Product> getByPropertyNameAndValue(List<Product> products,Map<String,String> nameValuePair ) throws InvalidParameterException {
	 ...
	 * 
	 * @param status : {@link ProductStatus}
	 * 
	 * @return List <{@link Product}> 
	 * 
	 */
	 ...
	 * 
	 * @param list : List <{@link Product}>
	 * @param pageNo : int
	 * @param productPerPage : int
	 * 
	 * @return List <{@link Product}> 
	 * 
	 */
    ...
    public static void setFileName(String fileName) {
		ProductService.fileName = fileName;
	}
