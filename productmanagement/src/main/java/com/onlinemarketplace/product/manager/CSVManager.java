...
public class CSVManager {
	...
	public boolean writeCSV(String fileName,List<Product> list){
		...
		try {
				...
				printer.printRecord("ID","PRODUCTCODE","NAME","CATEGORY","SUBCATEGORY","LISTINGPRICE","CASTPRICE","PRIORITY",
						"PREORDERSTATUS","AVAIBLEDATE","PRICE","QUANTITY","PROPERTIES");
				...
				productData.add(categoryService.getRepository().findOne(product.getProductCategory()).toString());
				productData.add(subCategoryService.getRepository().findOne(product.getProductSubCategory()).toString());
				productData.add(product.getListingPrice()+"");
				productData.add(product.getCastPrice()+"" );
				productData.add(product.getPriority().id+"" );
	...
	public List<Product> readCSV(String fileName){
		...
		try{
			...
			for(CSVRecord record : parser){
				...
				product.setCategory(categoryService.getRepository().getByName( record.get("CATEGORY") ) );
				product.setProductCategory( product.getCategory().getId() );
				product.setSubCategory( subCategoryService.getRepository().getByName(record.get("SUBCATEGORY")));
				product.setProductSubCategory(product.getSubCategory().getId() );
				...
				product.setPriority(Priority.getById( Integer.parseInt( record.get("PRIORITY") ) ) );
