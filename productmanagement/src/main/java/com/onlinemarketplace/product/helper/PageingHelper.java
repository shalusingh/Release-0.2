...
import com.onlinemarketplace.dto.helper.DTOHelperBase;
...
public class PageingHelper {
    ...
    /**
     * Calculate page valiables. Ex. get start and end point of a page.
     *
     * @param iPageCount
     *            : int
     * @param iPageNumber
    ...
    public static int[] pagingCounters(int iPageCount, int iPageNumber, int iTotalCount) {
        ...
        try {
            ...
            int iEnd = 0;
            int iTotalPageNumber = iTotalCount
                / iPageCount;
            if (iTotalCount
                % iPageCount != 0)
                iTotalPageNumber++;
            ...
            if (iPageCount > iTotalCount) {
                ...
                if (iPageCount
                    * iPageNumber > iTotalCount) {
                    if (iPageNumber > iTotalPageNumber)
                        iPageNumber = iTotalPageNumber;

                    int iTemp = iPageCount
                        * iPageNumber;
                    intArray[ProductConstants.START] = iTemp
                        - iPageCount;
                    intArray[ProductConstants.END] = iTotalCount;
                    ...
                    if (iPageNumber == 0) {
                        ...
                        intArray[ProductConstants.START] = iEnd
                            - iPageCount;
                    }
    ...
    public static Map<String, String> getMap(String queryString) throws InvalidParameterException {
        ...
        for (int i = 0; i < queryArray.length; i += 2) {
            requestMap.put(queryArray[i], queryArray[i
                + 1]);
        }
        return requestMap;
    }
    ...
    /**
     * This function convert pagenation domain object to dto object.
     *
     * @param base
     *            : {@link DTOHelperBase}
     * @param pagenationDomain
     *            : Pagenation;
     *
     * @return Pagenation : Dto Object.
     *
     */
    @SuppressWarnings({
        "unchecked", "rawtypes" })
    public static Pagenation convertDto(DTOHelperBase base, Pagenation pagenationDomain) {
        Pagenation pagenation = null;

        if (base instanceof ProductDtoHelper) {
            pagenation = new Pagenation<ProductDto>();
            try {
                pagenation.setiCount(pagenationDomain.getiCount());
                pagenation.setiEnd(pagenationDomain.getiEnd());
                pagenation.setiPageNo(pagenationDomain.getiPageNo());
                pagenation.setiPageSize(pagenationDomain.getiPageSize());
                pagenation.setiStart(pagenationDomain.getiStart());

                pagenation.setResult(((ProductDtoHelper) base).serializeAll(pagenationDomain.getResult()));

            } catch (NullPointerException exception) {
                logger.error("Unable to convert domain to DTO", exception);
            }
