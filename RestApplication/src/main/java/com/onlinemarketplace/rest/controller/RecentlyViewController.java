...
public class RecentlyViewController {
	...
	public @ResponseBody Response<Product> get(HttpSession session,HttpServletRequest request){
		...
		try{
			if(customer!=null){
			  recentlyViews = recntlyViewRepository.getByCustomerId(customer.getCustomerId());
			  if(recentlyViews!=null && !recentlyViews.isEmpty()){
	...
	public @ResponseBody Response<RecntlyView>  add(HttpServletRequest request, HttpServletResponse servletResponse,@PathVariable("pId") long productId,HttpSession session){
		...
		if(customer!=null){ 
			cookieName = cookieName+customer.getCustomerId(); 
			RecntlyView recntlyView = new RecntlyView();
			recntlyView.setCustomerId(customer.getCustomerId());
